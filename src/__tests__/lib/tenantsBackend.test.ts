/**
 * Tests for lib/api/tenantsBackend.ts
 *
...
import { backendCreateTenant } from "@/lib/api/tenantsBackend"
import { ApiError } from "@/types"
import type { TenantCreatePayload } from "@/types/tenant"
...
describe("tenantsBackend detail extraction", () => {
  it("uses a plain string detail as-is", async () => {
    ...
    try {
      ...
      expect((err as ApiError).detail).toBe("Tenant with this email already exists")
    }
  })
  ...
  it("joins FastAPI's structured 422 validation error array by msg", async () => {
    ...
    try {
      await backendCreateTenant("token", createPayload)
      throw new Error("expected backendCreateTenant to reject")
    } catch (err) {
